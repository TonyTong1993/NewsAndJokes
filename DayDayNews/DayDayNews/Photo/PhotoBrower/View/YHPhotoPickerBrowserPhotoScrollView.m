//
//  YHPhotoPickerBrowserPhotoScrollView.m
...
@interface YHPhotoPickerBrowserPhotoScrollView(){
    YHPhotoPickerBrowserPhotoView *_tapView; // for background taps
    CGFloat maxScale;
    CGFloat minScale;
...
@end
@implementation YHPhotoPickerBrowserPhotoScrollView
- (void)dealloc {
...
- (id)init{
    if ((self = [super init])) {
        ...
        _tapView = [[YHPhotoPickerBrowserPhotoView alloc] initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[YHPhotoPickerBrowserPhotoImageView alloc] initWithFrame:CGRectZero];
...
- (void)setPhoto:(YHPhotoBrowserPhoto *)photo{
    _photo = photo;
    ...
    if (photo.photoPath.length) {
        ...
        if (photoRange.location != NSNotFound){
            [[YHPhotoPickerDatas defaultPicker] getAssetsPhotoWithURLs:photo.photoURL callBack:^(UIImage *obj) {
            ...
            _photoImageView.frame = [YHPhotoRect setMaxMinZoomScalesForCurrentBoundWithImageView:_photoImageView];
