//
//  YHPhotoPickerBrowserViewController.m
...
@interface YHPhotoPickerBrowserViewController ()<UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate>
//@property (nonatomic, weak)  UILabel          *pageLabel;
//@property (nonatomic, weak)  UIButton         *backBtn;
@property (nonatomic, weak)  UICollectionView *collectionView;
@property (nonatomic, assign) NSInteger currentPage;
@property (nonatomic, assign) BOOL isNowRotation;
//@property (nonatomic, strong) YHPhotoPickerCustomToolBarView *XGtoolbar;
@property (nonatomic, weak) YHPhotoPickerBrowserPhotoScrollView *cellScrollView;
...
@implementation YHPhotoPickerBrowserViewController
...
//- (void)setShowType:(YHShowImageType)showType
//{
//    _showType = showType;
//    if (self.showType != YHShowImageTypeImagePicker){
...
- (void)setupCollectionView{
    if (!_collectionView) {
        ...
        flowLayout.minimumLineSpacing = YHPickerColletionViewPadding;
        flowLayout.minimumInteritemSpacing = 0;
        flowLayout.itemSize = self.view.size;
        flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        CGRect frame = self.view.bounds;
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width + YHPickerColletionViewPadding,self.view.height) collectionViewLayout:flowLayout];
...
//        NSDictionary *metrics = @{@"ZLPickerPageCtrlH":@(YHPickerPageCtrlH)};
//        
...
- (void)setupXGToolbar
{
    ...
    //        CGFloat height = 44;
    //        _XGtoolbar = [[YHPhotoPickerCustomToolBarView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - height, CGRectGetWidth(self.view.bounds), height) showType:self.showType];
    //        _XGtoolbar.delegate = self;
    //
    //        __weak YHPhotoPickerBrowserViewController * weakself = self;
    //
    //        _XGtoolbar.getSizeBlock = ^(){
    //            YHPhotoBrowserPhoto *photo = weakself.photos[weakself.currentPage];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        YHPhotoPickerBrowserPhotoScrollView *scrollView =  [[YHPhotoPickerBrowserPhotoScrollView alloc] init];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
    return CGSizeMake(YHPickerColletionViewPadding, self.view.frame.size.height);
}
...
//    YHPhotoAssets *photoAsset = ((YHPhotoBrowserPhoto *)self.photos[self.currentPage]).asset;
//
//    for (YHPhotoAssets *asset in self.selectedAssets) {
...
- (void)pickerPhotoScrollViewDidSingleClick:(YHPhotoPickerBrowserViewController *)photoScrollView{
...
- (void) pickerPhotoScrollViewDidLongPressed:(YHPhotoPickerBrowserViewController *)photoScrollView{
...
- (void)prepareLeftAndRighImage {
    if (self.photos.count > 1) {
        if (self.currentPage == 0) {//从第一张进入
            if (!((YHPhotoBrowserPhoto *)self.photos[1]).photoImage) {
                YHPhotoAssets *asset = ((YHPhotoBrowserPhoto *)self.photos[1]).asset;
                ((YHPhotoBrowserPhoto *)self.photos[1]).photoImage = [asset originImage];
            }
        } else if (self.currentPage == self.photos.count - 1) {//从第最后一张进入
            if (!((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage) {
                YHPhotoAssets *asset = ((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).asset;
                ((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage = [asset originImage];
            }
        } else {
            if (!((YHPhotoBrowserPhoto *)self.photos[self.currentPage + 1]).photoImage) {
                YHPhotoAssets *asset = ((YHPhotoBrowserPhoto *)self.photos[self.currentPage + 1]).asset;
            ...
            if (!((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage) {
                YHPhotoAssets *asset1 = ((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).asset;
                ((YHPhotoBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage = [asset1 originImage];
            }
        }
    }
...
- (void)longPressAction {
    ...
    [actionsheet addButtonWithTitle:@"保存"];
...
- (id<YHPhotoPickerBrowserPhoto>)currentPhoto{
